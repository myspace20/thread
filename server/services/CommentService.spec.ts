...
describe('CommentService', () => {
  ...
  describe('retrieve comment', () => {
    it('get comment by id', async () => {
      ...
      expect(commentRepositoryStub.getById.calledOnceWithExactly(id)).toEqual(true);
    });

    it('get comment by id -- error', async () => {
      let error;
      commentRepositoryStub.getById.throws(new HttpError(404, 'comment not found'));
      try {
  ...
  describe('create comments', () => {
    it('creates a comment for a thread', async () => {
      ...
      expect(threadRepositoryStub.getById.calledOnceWithExactly(dummyComment.thread_id)).toEqual(
        true,
      );
      expect(commentRepositoryStub.createComment.calledOnceWithExactly(dummyComment)).toEqual(true);
    });
    ...
    it('creates a comment for a post', async () => {
      ...
      expect(result).toEqual(dummyComment);
      expect(postRepositoryStub.getById.calledOnceWithExactly(dummyComment.post_id)).toEqual(true);
      expect(commentRepositoryStub.createComment.calledOnceWithExactly(dummyComment)).toEqual(true);
    });
  ...
  describe('edit comment', () => {
    it('updates the text field of a comment', async () => {
      ...
      const dummyComment = { id, post_id: id, user_id: id, text: 'to be updated' };
      const updatedTextResult = { id, post_id: id, user_id: id, text: 'updated' };
      ...
      const result = await commentService.editPostComment(dummyQuery, updatedTextData);
      expect(result).toEqual(updatedTextResult);
      expect(commentRepositoryStub.getByFilter.calledOnceWithExactly(dummyQuery)).toEqual(true);
      expect(
        commentRepositoryStub.editComment.calledOnceWithExactly(dummyComment.id, updatedTextData),
      ).toEqual(true);
    ...
    it('updates the text field of a comment -- error', async () => {
      let error;
      commentRepositoryStub.getByFilter.throws(new HttpError(404, 'post not found'));
      try {
  ...
  describe('remove comment', () => {
    it('removes a users comment', async () => {
      ...
      expect(commentRepositoryStub.getByFilter.calledOnceWithExactly(dummyQuery)).toEqual(true);
      expect(commentRepositoryStub.deleteComment.calledOnceWithExactly(dummyQuery.id)).toEqual(
        true,
      );
    });
  });
